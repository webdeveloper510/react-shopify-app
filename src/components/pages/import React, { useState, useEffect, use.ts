...
import { API, getInfluencerList, payInfluencer } from '../../config/Api';
...
const CreateInfluencer = () => {
    ...
    // const [influencer_list, setInfluencerList] = useState([]);
    ...
    const [open_modal, setOpenModal] = useState({ toggle: false, value: null, id: null })
    ...
    const handleBack = () => {
        ...
        setSelectedRows([]);
        const initialStates = {};
        influencer_list?.forEach((list, i) => {
    ...
    useEffect(() => {
        if (Array.isArray(prodInfluId) && Array.isArray(influencer_list)) {
            const matchedNames = [];

            prodInfluId.forEach((id) => {
                const matchingInfluencers = influencer_list.filter(
    ...
    return (
        <div className="campaign-new p-4 page">
                                ...
                                <div className='influencer-list-main'>
                                    {influencer_list?.map((list, i) => (
                                        <div className='influencer-list-container d-flex align-items-center justify-content-between'>
                                            ...
                                            <p className='d-flex flex-column align-items-center col-md-3'><strong>{(list?.follower / 1000000).toFixed(2)} M </strong> <span>Followers</span> </p>
                                            <p className='d-flex flex-column align-items-center col-md-3'><strong>{(list?.engagements / 1000000).toFixed(2) + "M"}<span className='ms-1'>({list.engagement_rate.toFixed(2)}%)</span></strong> <span>Engagement</span> </p>
                                            <div className='d-flex flex-column align-items-end col-md-2'><strong>AED {list?.influencerid_id__fee || "N/A"}</strong><button className='btn btn-dark' onClick={() => { setOpenModal({ toggle: true, value: { name: list?.fullname, cost: list?.influencerid_id__fee || "40" }, id: list?.influencerid_id }) }}>Pay</button></div>
                                        </div>
                                    ))}
                                </div>
            ...
            <PaymentModal data={open_modal} handler={(value) => { setOpenModal(value) }} />
        </div>
    );
}

const PaymentModal = ({ data, handler }) => {
            ...
            <Modal.Body >
                ...
                <div className='my-4'>
                    <Elements stripe={stripePromise}>
                        <InputElement payRef={payRef} handler={handler} data={data} />
                    </Elements>
                </div>
            </Modal.Body>
            <Modal.Footer>
...
const InputElement = ({ payRef, handler, data }) => {
